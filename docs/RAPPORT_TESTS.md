...
| -------------------------------- | ----------------------------------------------------------------------- | ------------------------------------ |
| Tests unitaires (Vitest)         | Traductions, contrastes, moteur d'apparence, validation, SEO, anti-abus | **206 / 206 réussis**                |
...
| `url.test.ts`                                         | Liens externes saisis dans le CMS : seules les adresses https:// absolues sont conservées (`http:`, `javascript:`, chemins relatifs, localhost refusés) ; affichage du domaine de destination ; site web d’une activité saisi sans « https:// » (www.exemple.com) complété automatiquement, http et https conservés, javascript:, mailto:, ftp:, localhost et identifiants refusés                         |
...
| `audit-log-view.test.ts`                              | Écran du journal d'audit : début des périodes jour, semaine (lundi), mois et année à minuit heure de Berlin, lecture sûre des filtres de l'adresse (valeurs inconnues ignorées, palier de 200 plafonné à 2 000), requête construite, adresse sans valeurs par défaut, regroupement par jour calendaire de Berlin, initiale de l'utilisateur                                                                |
...
| `brand-mark.test.ts`                                  | Logo : les deux moitiés de l'anneau et les lettres R et K dans leurs couleurs, espace entre les deux moitiés, logo décoratif sauf si un titre est fourni                                                                                                                                                                                                                                                   |
| `site-settings-credit.test.ts`                        | Crédit du pied de page : nom Nana-Consulting affiché en lecture seule, lien masqué, toute modification refusée, même à un administrateur                                                                                                                                                                                                                                                                   |
| `portrait-slideshow.test.tsx`                         | Diaporama du hero d'accueil : une seule photo sans commandes ; précédent, suivant et points de navigation ; défilement automatique et bouton pause ; aucun défilement automatique si l'utilisateur demande moins d'animations                                                                                                                                                                              |
| `nav-badges.test.ts`, `nav-badges-component.test.tsx` | Pastilles du menu de l'administration : aucune pastille à 0, « 99+ » au-delà ; info-bulle FR / DE / EN ; seules les demandes « Nouvelle » et les abonnés non désinscrits arrivés depuis la dernière visite sont comptés ; la visite de la liste des abonnés remet le compteur à zéro ; premier passage sans rattrapage des anciens abonnés ; pastille réappliquée quand le menu se redessine               |
| `dashboard-stats.test.ts`, `admin-dashboard.test.tsx` | Vue d'ensemble de l'administration : mois calendaires (passage d'année), libellés FR / DE / EN, longueur des barres, évolution par rapport au mois précédent, liens vers les listes filtrées ; rendu avec des chiffres fictifs (tuiles, graphiques, demande prioritaire liée à sa fiche), toutes les requêtes avec les droits de l'utilisateur ; rien sans utilisateur connecté ni en cas d'erreur de base |
...
- Demande adressée à une entreprise : `/en/contact?business=rk-business-consulting` présélectionne
  l'entreprise ; après avoir donné un e-mail de contact à l'entreprise, une demande envoyée pour
  elle est enregistrée avec la relation et la notification arrive dans MailHog à cette adresse,
  avec une copie ; l'e-mail de l'entreprise est ensuite rétabli (projet Chromium seulement)
...
### Vue d'ensemble de l'administration (`admin-dashboard.spec.ts`)
...
- Écran du journal : 5 onglets de période dont un actif, jour le plus récent ouvert avec la
  connexion du test, onglet « Jour » puis filtre « Connexion » conservés dans l'adresse, seules
  des connexions ou déconnexions affichées
- Écran du journal : 5 onglets de période dont un actif, jour le plus récent ouvert avec la
  connexion du test, onglet « Jour » puis filtre « Connexion » conservés dans l'adresse, seules
  des connexions ou déconnexions affichées
